...
// Fully-unpaid signals only. Revenue-share / profit-share are allowed (paid +
// rev-share counts as paid) — they are intentionally NOT listed here.
const UNPAID_RE =
  /(unpaid|for exposure|for credit|\bno pay\b|no budget|volunteer|for free|spec work|portfolio only)/i;

// Requests that have been closed/filled (often tagged in the title or URL slug).
const CLOSED_RE = /(\[closed\]|\bclosed\b|\bfilled\b|position filled|no longer (available|needed)|\bfulfilled\b)/i;
...
export function cleanLeads(leads, opts = {}) {
  ...
  const dropped = { host: 0, generic: 0, keyword: 0, unpaid: 0, closed: 0, badurl: 0 };
  const kept = leads.filter((l) => {
    ...
    // Include the URL slug for unpaid/closed checks — tags often live only there.
    const textUrl = `${text} ${(l.url || "").toLowerCase()}`;
    if (kwRes.some((re) => re.test(text))) { dropped.keyword++; return false; }
    if (CLOSED_RE.test(`${l.title || ""} ${l.url || ""}`)) { dropped.closed++; return false; }
    if (paidOnly && UNPAID_RE.test(textUrl)) { dropped.unpaid++; return false; }
    return true;
  });
  return { kept, dropped };
}
